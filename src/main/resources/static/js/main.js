$(document).ready(function() {
	...
	// ========================================================================
	// SafeMargin Fit — 모든 요소를 정중앙 기준 균일 축소하여 안전선 안으로 맞춤
	// ========================================================================

	/**
	 * 모든 요소의 relativeState를 페이지 중심(50%, 50%) 기준으로 균일 축소
	 * @param {number} oldMargin - 기존 여백 (mm), 예: 3
	 * @param {number} newMargin - 새 여백 (mm), 예: 6
	 * @returns {{ scaled: boolean, count?: number, scale?: number, reason?: string }}
	 */
	window.safeMarginFit = function(oldMargin, newMargin) {
		if (!window.safeLineManager) {
			console.error('[safeMarginFit] SafeLineManager not initialized');
			return { scaled: false, reason: 'no_safe_line_manager' };
		}

		const scale = window.safeLineManager.getMarginFitScale(oldMargin, newMargin);

		console.log(`[safeMarginFit] Scaling from ${oldMargin}mm to ${newMargin}mm, scale=${scale.toFixed(8)}`);

		if (Math.abs(scale - 1.0) < 0.0001) {
			console.log('[safeMarginFit] Scale is ~1.0, nothing to do');
			return { scaled: false, reason: 'scale_is_unity' };
		}

		const CENTER = 50; // 페이지 중심 (백분율 좌표)
		let processedCount = 0;

		// 헬퍼: 백분율 좌표를 페이지 중심 기준으로 축소
		function scaleFromCenter(val) {
			return CENTER + (val - CENTER) * scale;
		}

		// 헬퍼: relativeState의 공통 필드 축소 처리
		function scaleRelativeState(rs) {
			// 위치 축소 (중심 기준)
			rs.position.left = scaleFromCenter(rs.position.left);
			rs.position.top  = scaleFromCenter(rs.position.top);

			// 크기 축소 (비례)
			rs.size.width  = rs.size.width  * scale;
			rs.size.height = rs.size.height * scale;

			// translate 축소 (상대 오프셋이므로 선형 비례)
			if (typeof rs.translateX === 'number') rs.translateX = rs.translateX * scale;
			if (typeof rs.translateY === 'number') rs.translateY = rs.translateY * scale;

			// 회전 중심점 축소 (존재할 때만)
			if (rs.center) {
				rs.center.x = scaleFromCenter(rs.center.x);
				rs.center.y = scaleFromCenter(rs.center.y);
			}

			// rotation, transformOriginX, transformOriginY: 변경 없음

			// 정렬 플래그 초기화 (축소 후 기존 정렬 좌표 무효)
			rs.alignment = { horizontal: null, vertical: null };
			rs.alignmentBounds = null;
		}

		// ---- frame-group 처리 (사진 프레임, 텍스트박스 프레임, 요소) ----
		$('#frame-container .frame-group').each(function() {
			const $el = $(this);
			const rs = $el.data('relativeState');
			if (!rs || !rs.position || !rs.size) return;

			scaleRelativeState(rs);
			$el.data('relativeState', rs);
			processedCount++;
		});
		...
		$('#frame-container .text-box').each(function() {
			...
			// 텍스트 전용: base-font-size도 비례 축소
			const oldBase = $el.data('base-font-size');
			if (typeof oldBase === 'number' && oldBase > 0) {
				const newBase = Math.round(oldBase * scale * 100) / 100;
				$el.data('base-font-size', newBase);
				console.log(`[safeMarginFit] text base-font-size: ${oldBase} → ${newBase}`);
			}
		...
		console.log(`[safeMarginFit] Processed ${processedCount} element(s), scale=${(scale * 100).toFixed(4)}%`);
		return { scaled: true, count: processedCount, scale: scale };
	};

	// SafeFit / Restore 버튼 표시/숨김 관리
	// @param {object} pageData - 서버에서 받은 페이지 데이터 (hasBackup 포함)
	window.showSafeFitButtonIfNeeded = function(pageData) {
		const $btnFit = $('#btn-safe-margin-fit');
		const $btnRestore = $('#btn-restore-backup');

		const userSafeMargin = parseInt($('#userSafeMargin').val());
		if (userSafeMargin !== 6) {
			$btnFit.hide();
			$btnRestore.hide();
			return;
		...
		const hasElements =
			$('#frame-container .frame-group').length > 0 ||
			$('#frame-container .text-box').length > 0;

		const hasBackup = pageData && pageData.hasBackup === true;

		// SafeFit 버튼: 요소 존재 시 표시, 백업 있으면 이미 적용된 것이므로 비활성화
		if (hasElements) {
			$btnFit.show();
			if (hasBackup) {
				$btnFit.prop('disabled', true).text('SafeFit ✓');
			} else {
				$btnFit.prop('disabled', false).text('SafeFit');
			}
		} else {
			$btnFit.hide();
		}

		// Restore 버튼: 백업 존재 시 표시
		if (hasBackup) {
			$btnRestore.show();
		} else {
			$btnRestore.hide();
	...
	// SafeFit 버튼 클릭 핸들러
	$(document).on('click', '#btn-safe-margin-fit', function(e) {
		e.preventDefault();
		...
		// yearbookId 확인 (백업을 위해 저장된 페이지여야 함)
		const yearbookId = activePageThumb ? activePageThumb.attr('data-yearbook-id') : null;
		if (!yearbookId) {
			alert('페이지를 먼저 저장한 후 SafeFit을 실행하세요.');
			return;
		}

		// 확인 다이얼로그
		if (!confirm(
			'모든 요소를 6mm 안전선 안으로 축소 맞춤합니다.\n' +
			'위치와 크기가 약 2.8% 축소됩니다.\n' +
			'현재 저장된 레이아웃이 백업됩니다.\n\n' +
			'진행하시겠습니까?'
		)) {
		...
		// ① 서버에 현재 designData 백업 요청
		$.ajax({
			url: `${ctx}/edit/backupDesignData`,
			method: 'POST',
			contentType: 'application/json',
			data: JSON.stringify({ yearbookId: +yearbookId }),
			success: function(res) {
				if (!res.success) {
					alert('백업 실패: ' + (res.message || '알 수 없는 오류'));
					$btn.prop('disabled', false).text('SafeFit');
					return;
				}

				console.log('[SafeFit] 백업 완료, 축소 시작');

				// ② 선택 해제
				if (window.selectionManager) window.selectionManager.clearSelection();
				if (window.multiSelectionManager) window.multiSelectionManager.clearSelection();

				// ③ 모든 요소의 relativeState 최신화
				$('#frame-container .frame-group').each(function() {
				...
				// ④ 축소 실행
				const result = window.safeMarginFit(3, 6);

				if (result.scaled) {
					$btn.prop('disabled', true).text('SafeFit ✓');
					$('#btn-restore-backup').show();

					alert(
						`${result.count}개 요소를 6mm 안전선에 맞춤 완료\n` +
						`축소율: ${(result.scale * 100).toFixed(2)}%\n\n` +
						'Save를 눌러 저장하세요.'
					);
				}
			},
			error: function(jqXHR) {
				const msg = jqXHR.responseJSON?.message || '서버 오류';
				alert('백업 실패: ' + msg);
				$btn.prop('disabled', false).text('SafeFit');
			}
		});
	...
	// Restore 버튼 클릭 핸들러 — 백업된 원본 레이아웃 복원
	$(document).on('click', '#btn-restore-backup', function(e) {
		e.preventDefault();

		const yearbookId = activePageThumb ? activePageThumb.attr('data-yearbook-id') : null;
		if (!yearbookId) {
			alert('복원할 페이지를 찾을 수 없습니다.');
			return;
		}

		if (!confirm(
			'SafeFit 적용 전의 원본 레이아웃으로 복원합니다.\n' +
			'SafeFit 이후 편집한 내용은 사라집니다.\n\n' +
			'복원하시겠습니까?'
		)) {
			return;
		}

		const $btnRestore = $(this);
		$btnRestore.prop('disabled', true).text('복원 중...');

		$.ajax({
			url: `${ctx}/edit/restoreDesignData`,
			method: 'POST',
			contentType: 'application/json',
			data: JSON.stringify({ yearbookId: +yearbookId }),
			success: function(res) {
				if (!res.success) {
					alert('복원 실패: ' + (res.message || '알 수 없는 오류'));
					$btnRestore.prop('disabled', false).text('Restore');
					return;
				}

				console.log('[Restore] 원본 복원 완료, 페이지 재렌더링');

				// 복원된 데이터로 페이지 재렌더링
				const restoredPageData = {
					designData: res.designData,
					lastSaved: res.lastSaved
				};

				renderPage(restoredPageData, function() {
					hideLoader();
					// 백업이 삭제되었으므로 Restore 숨김, SafeFit 재활성화
					$('#btn-restore-backup').hide();
					$('#btn-safe-margin-fit').show().prop('disabled', false).text('SafeFit');

					if (res.lastSaved) {
						displayLastSaveTime(res.lastSaved);
					}

					alert('원본 레이아웃이 복원되었습니다.');
				});
			},
			error: function(jqXHR) {
				const msg = jqXHR.responseJSON?.message || '서버 오류';
	...
	$('.content').on('click', '.edit-btn', async function(e) {
		...
		try {
			...
			renderPage(pageData, function() {
				hideLoader();
				// 렌더링 완료 후 SafeFit / Restore 버튼 표시 여부 결정
				if (window.showSafeFitButtonIfNeeded) {
				...
				const promise = new Promise((resolve) => {
					...
					maskImg.onload = async function() {
						try {
							...
							// ✅ 이미지 디코딩 완료 대기 (decode 미완료 시 drawImage가 조용히 스킵되어 갭 방지)
							try { await photoImg.decode(); } catch (e) { /* already decoded or unsupported */ }
